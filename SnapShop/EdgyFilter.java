/**
 * 
 * This class applies the filter "Edgy" to the PixelImage
 * @author Samson Haile
 * @version Standard
 * 
 */
public class EdgyFilter implements Filter{
  ...
  /**
   * 
   * This method contains the algorithm to process the pixels to create the "Edgy" effect
   ...
   */
   public void filter (PixelImage theImage){
      ...
      for (int row = 1; row < theImage.getHeight()-1; row++){
           for (int col = 1; col < theImage.getWidth()-1; col++) {
                for (int i = 0; i < 3; i++){
                     ...
                     //These variables save extract the rgb color values
    ...
    // update the image with the changed pixels
